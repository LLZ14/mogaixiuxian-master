...
export class SecretPlaceplus extends plugin {
    ...
    async Gosecretplace(e) {
     ...
     async Goshijie(e) {
        ...
        now_level_id = data.shijie_list.find(item => item.level_id == player.level_id).level_id;
        if (now_level_id < 22) {
            e.reply("世界守门人:再去变强点再来吧！")
            return;
        }
        let didian = await e.msg.replace("#沉迷世界", '');
        let code = didian.split("\*");
        didian = code[0];
        let i = code[1];
        if (i < 1 || i == null || i == undefined || i == NaN) {
